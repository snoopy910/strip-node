			...
			if operation.Status == OPERATION_STATUS_PENDING {
				...
				if operation.Type == OPERATION_TYPE_TRANSACTION {
					...
					if depositOperation.KeyCurve == "ecdsa" {
						...
						depositOperation.KeyCurve == "bitcoin_ecdsa" || depositOperation.KeyCurve == "stellar_eddsa" || depositOperation.KeyCurve == "algorand_eddsa" || depositOperation.KeyCurve == "ripple_eddsa" {
						chain, err := common.GetChain(operation.ChainId)
						if err != nil {
							fmt.Println(err)
							break
