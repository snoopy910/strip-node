...
func handleIncomingMessage(message []byte) {
	...
	instance := getSignerHubContract(networks[msg.NetworkId].RPC_URL, networks[msg.NetworkId].SignerHubContractAddress)
	...
	if msg.Type == MESSAGE_TYPE_GENERATE_START_KEYGEN {
		go generateKeygen(msg.NetworkId)
	} else if msg.Type == MESSAGE_TYPE_GENERATE_KEYGEN {
		go updateKeygen(msg.NetworkId, "keygen", msg.From, msg.Message, msg.IsBroadcast, msg.To)
	} else if msg.Type == MESSAGE_TYPE_START_SIGN {
		//validate if the hash if from paymaster
		if verifyhash {
			resp, err := http.Get(networks[msg.NetworkId].PaymasterURL + "/isValid?hash=" + string(msg.Hash))
			if err == nil {
		...
		go generateSignature(msg.NetworkId, msg.Hash)
	} else if msg.Type == MESSAGE_TYPE_SIGN {
		go updateSignature(msg.NetworkId, string(msg.Hash), msg.From, msg.Message, msg.IsBroadcast, msg.To)
...
func broadcast(message Message) {
	...
	cleanedPrivateKey := strings.Replace(nodeKey.PrivateKey, "0x", "", 1)
